...
def create_multiple_encodings_from_single_image(image_path):
    """
    ...
    # 기준 이미지에서 얼굴 감지 (처음 한 번은 확실하게)
    # CNN 모델 사용 (더 정확하지만 느림, 기준 이미지는 한 번만 처리되므로 감수할만함)
    print("[INFO] 기준 이미지에서 얼굴 감지 중 (CNN 모델 사용)...")
    start_time_detection = time.time()  # 감지 시작 시간 기록
    face_locations = face_recognition.face_locations(
        enhanced_image, model="cnn", number_of_times_to_upsample=2)  # 2배 업샘플링으로 작은 얼굴도 감지
...
def advanced_face_detection(frame):
    """
    단일 프레임에서 여러 방법을 사용하여 얼굴을 감지하고 인코딩합니다.
    이후 IoU(Intersection over Union) 기반으로 중복된 얼굴을 제거하여
    고유한 얼굴 감지 결과만 반환합니다.
    """
    rgb_frame = cv2.cvtColor(
        frame, cv2.COLOR_BGR2RGB)  # OpenCV는 BGR, face_recognition은 RGB를 기대

    all_face_locations = []  # 모든 감지 방법에서 얻은 얼굴 위치
    all_face_encodings = []  # 모든 감지 방법에서 얻은 얼굴 인코딩

    # 방법 1: 원본 이미지 + HOG (빠르고 일반적, 기본 감지)
    # number_of_times_to_upsample=1은 작은 얼굴 감지에 도움을 줍니다.
    locations_hog = face_recognition.face_locations(
        rgb_frame, model="hog", number_of_times_to_upsample=1)
    if locations_hog:
        encodings_hog = face_recognition.face_encodings(
            rgb_frame, locations_hog)
        all_face_locations.extend(locations_hog)
        all_face_encodings.extend(encodings_hog)

    # 방법 2: 향상된 이미지 + HOG (명암 대비 개선으로 감지율 향상)
    enhanced_frame = enhance_image_quality(rgb_frame)
    locations_enhanced = face_recognition.face_locations(
        enhanced_frame, model="hog", number_of_times_to_upsample=1)
    if locations_enhanced:
        encodings_enhanced = face_recognition.face_encodings(
            enhanced_frame, locations_enhanced)
        all_face_locations.extend(locations_enhanced)
        all_face_encodings.extend(encodings_enhanced)

    # 방법 3: 스케일 조정 후 감지 (작은 얼굴 또는 멀리 있는 얼굴 대응)
    scale_factor = 1.5  # 이미지를 1.5배 확대하여 작은 얼굴 감지 시도
    scaled_frame = cv2.resize(
        rgb_frame, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_LINEAR)
    locations_scaled = face_recognition.face_locations(
        scaled_frame, model="hog", number_of_times_to_upsample=0)  # 이미 스케일업 했으므로 업샘플링은 0

    if locations_scaled:
        # 스케일링된 이미지에서의 좌표를 원본 이미지 좌표로 변환
        locations_scaled_original_coords = [
            (int(top / scale_factor), int(right / scale_factor),
             int(bottom / scale_factor), int(left / scale_factor)) for top, right, bottom, left in locations_scaled
        ]
        # 원본 이미지에서 인코딩 수행 (정확한 인코딩을 위해)
        encodings_scaled = face_recognition.face_encodings(
            rgb_frame, locations_scaled_original_coords)
        all_face_locations.extend(locations_scaled_original_coords)
        all_face_encodings.extend(encodings_scaled)
    ...
    # (위치, 인코딩) 쌍으로 묶어 처리
    faces_data = list(zip(all_face_locations, all_face_encodings))

    iou_threshold = 0.3  # IoU 임계값 (0.3 이상 겹치면 동일한 얼굴로 간주)

    for i, (loc1, enc1) in enumerate(faces_data):
        is_duplicate = False
        # 이미 고유한 얼굴 리스트에 있는 얼굴들과 비교
        for j, (loc2, enc2) in enumerate(unique_face_locations):
            if iou(loc1, loc2) > iou_threshold:  # IoU가 임계값보다 크면 중복
                is_duplicate = True
                break

        if not is_duplicate:  # 중복이 아니면 고유한 리스트에 추가
            unique_face_locations.append(loc1)
            unique_face_encodings.append(enc1)

    return unique_face_locations, unique_face_encodings
...
def process_video_enhanced_single_ref(video_path, reference_img_path, interval=1, tolerance=0.55):
    ...
    progress = tqdm(total=total_frames, desc="🎞 향상된 영상 분석 중", unit="frame")
    ...
    while cap.isOpened():
        ...
        if frame_count % frame_interval == 0:
            # 고급 얼굴 감지 적용
            face_locations, face_encodings = advanced_face_detection(frame)
